...
class Ruleset(models.Model):
    ...
    match_key = models.CharField(
        max_length=255, help_text="Field name to use for matching records between source and target"
    )
...
class ReconciliationJob(models.Model):
    ...
    ruleset = models.ForeignKey(
        Ruleset, on_delete=models.CASCADE, related_name="jobs", null=True, blank=True
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
