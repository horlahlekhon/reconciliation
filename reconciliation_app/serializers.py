...
class ReconciliationJobSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'ruleset_id', 'ruleset_name', 'source_file', 'target_file', 'status', 'created_at', 'updated_at', 'result_summary', 'error_message']
        read_only_fields = ['id', 'status', 'created_at', 'updated_at', 'result_summary', 'error_message', 'ruleset_name']
    ...
    def validate_and_count_csv_files(cls, source_file, target_file, ruleset):
        ...
        try:
            ...
            required_fields = set(field.field_name for field in ruleset.fields.filter(is_required=True))
            ...
            if ruleset.match_key not in source_headers:
                ...
                raise ValueError(f"Match key '{ruleset.match_key}' not found in source file headers")
            if ruleset.match_key not in target_headers:
                logger.error(f"Match key '{ruleset.match_key}' not found in target file headers")
                raise ValueError(f"Match key '{ruleset.match_key}' not found in target file headers")
            ...
            if source_missing_required:
                error_msg = f"Source file missing required fields: {', '.join(source_missing_required)}"
                logger.error(error_msg)
                validation_errors.append(error_msg)
            if target_missing_required:
                error_msg = f"Target file missing required fields: {', '.join(target_missing_required)}"
                logger.error(error_msg)
                validation_errors.append(error_msg)
            ...
            logger.info(f"CSV validation successful - Source: {len(source_data)} records, "
...
class ReconciliationResultSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ['id', 'result_type', 'source_row_data', 'target_row_data', 'match_key', 'differences']
...
class ReconciliationJobDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ReconciliationJob
        fields = ['id', 'ruleset_id', 'ruleset_name', 'match_key', 'status', 'created_at', 'updated_at', 'result_summary', 'error_message']
...
class RulesetListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Ruleset
        fields = ['id', 'name', 'description', 'match_key', 'created_at', 'updated_at', 'fields_count']
        read_only_fields = ['id', 'created_at', 'updated_at']
