...
class ReconcileCSVFilesView(APIView):
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                'source_file': openapi.Schema(type=openapi.TYPE_FILE, description='Source CSV file'),
                'target_file': openapi.Schema(type=openapi.TYPE_FILE, description='Target CSV file'),
                'ruleset_id': openapi.Schema(type=openapi.TYPE_STRING, format=openapi.FORMAT_UUID, description='Ruleset ID for validation'),
            },
    ...
    def post(self, request):
        logger.info(f"Reconciliation request received from {request.META.get('REMOTE_ADDR', 'unknown')}")
        
        source_file = request.FILES.get('source_file')
        target_file = request.FILES.get('target_file')
        ruleset_id = request.data.get('ruleset_id')
        
        logger.debug(f"Request parameters: source_file={source_file.name if source_file else None}, "
        ...
        validation_result = ReconciliationJobSerializer.validate_and_count_csv_files(source_file, target_file, ruleset)
        ...
        try:
            ...
            logger.info(f"Created job {job.id} with ruleset {ruleset.name} - "
...
class JobResultsView(APIView):
    @swagger_auto_schema(
        ...
        manual_parameters=[
            ...
            openapi.Parameter('result_type', openapi.IN_QUERY, description="Filter by result type (matched, unmatched_source, unmatched_target)", type=openapi.TYPE_STRING),
        ],
        responses={
    ...
    def get(self, request, job_id):
        ...
        logger.debug(f"Job results requested for job {job_id} - "
        ...
        if result_type and result_type in ['matched', 'unmatched_source', 'unmatched_target', 'duplicate']:
